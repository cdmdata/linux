...
static void disable_mclk(struct snd_soc_codec *codec)
{
	struct sgtl5000_priv *sgtl5000 = snd_soc_codec_get_drvdata(codec);

	pr_debug("%s SACR0=%x SADIV=%x\n", __func__, SACR0, SADIV);
	if (sgtl5000->i2s_enabled) {
		sgtl5000->i2s_enabled = 0;
		if (sgtl5000->clk_i2s)
			clk_disable(sgtl5000->clk_i2s);
	}
}

static void enable_mclk(struct snd_soc_codec *codec)
{
	struct sgtl5000_priv *sgtl5000 = snd_soc_codec_get_drvdata(codec);

	pr_debug("%s SACR0=%x SADIV=%x\n", __func__, SACR0, SADIV);
	if (!sgtl5000->i2s_enabled) {
		sgtl5000->i2s_enabled = 1;
		if (!sgtl5000->clk_i2s) {
			struct clk *clk_i2s;
			clk_i2s = clk_get(codec->dev, NULL);
			if (IS_ERR(clk_i2s)) {
				pr_debug("%s clk_get error for I2SCLK\n", __func__);
				return;
			}
			sgtl5000->clk_i2s = clk_i2s; 
			SACR0 = SACR0_BCKD | SACR0_RFTH(14) | SACR0_TFTH(1);
			SACR1 = SACR1_DRPL | SACR1_DREC;
			SADIV = 0x0c;	//SYS_MCLK has allowed range of 8-27 MHz, 0x0c gives 12.235 MHz (table 14-2 page 719) 8Mhz/256 = 31.25 kHz
		}
		clk_enable(sgtl5000->clk_i2s);
	}
}
				     ...
				     unsigned int reg)
{
	...
	if (i2c_ret < 0) {
		pr_err("%s: read reg error : Reg 0x%02x, ret=%i SACR0=%x SADIV=%x\n", __func__, reg, i2c_ret, SACR0, SADIV);
		return 0;
	}
			  ...
			  unsigned int value)
{
	...
	if (i2c_ret < 0) {
		pr_err("%s: write reg error : Reg 0x%02x = 0x%04x SACR0=%x SADIV=%x\n",
		       __func__, reg, value, SACR0, SADIV);
		return -EIO;
	}
...
//		pr_err("%s: in: %d, int_div=%d, frac_div=%d div2=%d\n", __func__, in, int_div, frac_div, div2);
