...
static int get_dma_channels(struct mtd_info *mtd, struct d_nand_chip *chip)
{
	...
	r = edma_oob_link = edma_alloc_slot(EDMA_SLOT_ANY);
	if (r < 0)
		goto out2;

	r = edma_ecc = edma_alloc_channel(EDMA_CHANNEL_ANY, "nand_ecc",
		NULL, NULL, queue_no);
	if (r < 0)
		goto out3;

	r = edma_fcr = edma_alloc_channel(EDMA_CHANNEL_ANY, "nand_fcr",
		NULL, NULL, queue_no);
	if (r < 0)
		goto out4;

	r = edma_ecc_link = edma_alloc_slot(EDMA_SLOT_ANY);
	if (r < 0)
		goto out5;

	r = edma_fcr_link = edma_alloc_slot(EDMA_SLOT_ANY);
	if (r < 0)
		goto out6;

#ifdef USE_DMA_FOR_WRITE
	r = edma_oob_write = edma_alloc_slot(EDMA_SLOT_ANY);
	if (r < 0)
		goto out7;
 ...
 */
	...
	edma_set_dest_params(edma, chip->bounce_phys, INCR, W32BIT);
...
/* Intialize oob transfer */
	edma_set_src_params(edma_oob_link, chip->mem_start, INCR, W32BIT);
	edma_set_src_index(edma_oob_link, 0, 0);
	edma_set_dest_params(edma_oob_link,
