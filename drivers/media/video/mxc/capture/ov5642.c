...
static struct reg_value ov5642_af_firmware_settings[] = {
 {0x8000,0x02,0,0},
 {0x8001,0x00,0,0},
 {0x8002,0x06,0,0},
 {0x8003,0x02,0,0},
...
static int ov5642_set_rotate_mode(struct reg_value *rotate_mode)
{
	...
	if (rotate_mode->u8Val==0xc1)
	{
/*	mdelay(400);	
        RegAddr=REG_STA_FOCUS; 
	retval = ov5642_read_reg(RegAddr, &StatReg);
                        if (retval < 0) {
                                pr_err("%s, read reg 0x%x failed\n", __FUNCTION__, RegAddr);
                        }

	pr_info(" >>>>>>>>>>>>>>>>>> Initial   %s      %x  %x\n",__func__,StatReg, rotate_mode->u8Val);

//Check status. Wait for S_IDLE
	...
	while(StatReg != S_IDLE);

//Send Singlefocusmode
*/
	RegAddr = REG_CMD_MAIN;
        Val=Singlefocusmode;
        retval = ov5642_write_reg(RegAddr, Val);
	...
	do
	{
        ...
        pr_info(" >>>>>>>>>>>>>>>>>> REG_STA_FOCUS after set to SingleFocus   %s      %x\n",__func__,StatReg);
	} while(lc++<4);
}
...
/*	pModeSetting = ov5642_af_firmware_settings;
	iModeSettingArySize = ARRAY_SIZE(ov5642_af_firmware_settings);
	pr_info("<><><><><><><><>< Doing Comparison!!!!!!!!!!\n");
	for (i = 0; i < iModeSettingArySize; ++i, ++pModeSetting) {
		/* Do comparison */
...
/*		RegAddr = pModeSetting->u16RegAddr;
		...
		if(RegVal != Val)
			pr_info("Miscompare at 0x%x Was 0x%x SB 0x%x\n", RegAddr, RegVal, Val);
