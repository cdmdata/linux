...
s32 init_recv_priv(struct recv_priv *precvpriv, _adapter *padapter)
{
	int i;
	...
	if (precvpriv->pallocated_recv_buf==NULL) {
		RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,("alloc recv_buf fail!\n"));
		res = _FAIL;
		goto exit;
	}
	_memset(precvpriv->pallocated_recv_buf, 0, NR_RECVBUFF *sizeof(struct recv_buf) + 4);
	precvpriv->precv_buf = ALIGN_ADDR(precvpriv->pallocated_recv_buf, 4);

	precvbuf = (struct recv_buf*)precvpriv->precv_buf;     
	for(i=0; i < NR_RECVBUFF; i++)
	{
		init_recvbuf(padapter, precvbuf);
		...
		// data would be modified or used in os_recvbuf_resource_alloc
		// must be initialized above!
		res = os_recvbuf_resource_alloc(padapter, precvbuf);
		if (res == _FAIL) goto exit;

		// data wouldn't be used in os_recvbuf_resource_alloc
	...
	// Prepare one receive buffer for drop rx data
	precvpriv->precv_buf_for_drop = _malloc(sizeof(struct recv_buf));
	if (precvpriv->precv_buf_for_drop == NULL) {
		RT_TRACE(_module_hci_intfs_c_, _drv_err_,
			 ("init_recv_priv: malloc for recv_buf_for_drop FAIL!\n"));
		res = _FAIL;
		goto exit;
	}
	_memset(precvpriv->precv_buf_for_drop, 0, sizeof(struct recv_buf));

	precvbuf = (struct recv_buf*)precvpriv->precv_buf_for_drop;

	precvbuf->pallocated_buf = _malloc(MAX_RECVBUF_SZ + 4);
	if (precvbuf->pallocated_buf == NULL) {
		RT_TRACE(_module_hci_intfs_c_, _drv_emerg_,
			 ("init_recv_priv: malloc for recv_buf_for_drop(inner) FAIL!\n"));
		_mfree((u8*)precvbuf, sizeof(struct recv_buf));
		precvpriv->precv_buf_for_drop = NULL;
		res = _FAIL;
		goto exit;
	}
	precvbuf->pbuf = ALIGN_ADDR(precvbuf->pallocated_buf, 4);
	init_recvbuf(padapter, precvbuf);
	precvbuf->adapter = padapter;

#ifdef PLATFORM_OS_XP
	precvbuf->mdl_ptr = IoAllocateMdl(precvbuf->pbuf, MAX_RECVBUF_SZ, FALSE, FALSE, NULL);
	if (precvbuf->mdl_ptr == NULL)
	{
		RT_TRACE(_module_hci_intfs_c_, _drv_emerg_,
			 ("init_recv_priv: IoAllocateMdl FAIL!\n"));

		precvbuf->pbuf = NULL;
		_mfree((u8*)precvbuf->pallocated_buf, MAX_RECVBUF_SZ + 4);
	...
	MmBuildMdlForNonPagedPool(precvbuf->mdl_ptr);
#endif
#endif
...
void free_recv_priv (struct recv_priv *precvpriv)
{
	...
	if (precvbuf) {
		...
		os_recvbuf_resource_free(padapter, precvbuf);
			_spinlock_free(&precvbuf->recvbuf_lock);
...
#ifdef CONFIG_SDIO_HCI
	// free the buffer for rx drop usage
	if (precvpriv->precv_buf_for_drop)
	{
		precvbuf = (struct recv_buf*)precvpriv->precv_buf_for_drop;

#ifdef PLATFORM_OS_XP
		if (precvbuf->mdl_ptr) {
			IoFreeMdl(precvbuf->mdl_ptr);
			precvbuf->mdl_ptr = NULL;
		}
#endif
		precvbuf->pbuf = NULL;
		if (precvbuf->pallocated_buf) {
			_mfree(precvbuf->pallocated_buf, MAX_RECVBUF_SZ + 4);
			precvbuf->pallocated_buf = NULL;
		}
...
s32 init_recvbuf(_adapter *padapter, struct recv_buf *precvbuf)
{
	s32 res = _SUCCESS;
	...
	if (precvbuf != NULL)
	{
	 ...
	 if(precvbuf->ref_cnt == 0 ){
			...
			RT_TRACE(_module_rtl8712_recv_c_, _drv_notice_,
				 ("free_recvframe: precvbuf=0x%p enqueue recv buf cnt=%d\n",
				  precvbuf, precvpriv->free_recv_buf_queue_cnt));
		} else {
			RT_TRACE(_module_rtl8712_recv_c_, _drv_notice_,
				 ("free_recvframe: precvbuf=0x%p ref_cnt=%d\n",
				  precvbuf, precvbuf->ref_cnt));
        }
...
void update_recvframe_attrib_from_recvstat(struct rx_pkt_attrib *pattrib, struct recv_stat *prxstat)
{
	...
	if(drvinfo_sz)
	{
		...
		RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("\n prxstat=%p  pphy_stat=%p \n",prxstat,pphy_stat));
...
#ifdef PLATFORM_LINUX
static void recvframe_expand_skb(_adapter *adapter, union recv_frame *prframe)
{
	struct recv_frame_hdr *pfhdr = &prframe->u.hdr;

	_pkt *skb = NULL;
	u32 alloc_sz = 1658;	// 1658 + 6 = 1664, 1664 is 128 alignment.
	u8 shift_sz = 0;
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,18)) // http://www.mail-archive.com/netdev@vger.kernel.org/msg17214.html
	skb = dev_alloc_skb(alloc_sz);
#else
	skb = netdev_alloc_skb(adapter->pnetdev, alloc_sz);
#endif
	if (!skb) return; // no way to expand

	//3 2. Prepare new skb and replace & release old skb
	// do 4 bytes alignment
	skb_reserve(skb, 4 - ((addr_t)skb->data & 0x3));
	skb_reserve(skb, shift_sz);

	_memcpy(skb_put(skb, pfhdr->len), pfhdr->rx_data, pfhdr->len);
	dev_kfree_skb_any(pfhdr->pkt);
	pfhdr->pkt = skb;
	pfhdr->rx_head = pfhdr->rx_data = skb->data;
#ifdef NET_SKBUFF_DATA_USES_OFFSET
	pfhdr->rx_tail = skb->head + skb->tail; //skb_tail_pointer(skb);
	pfhdr->rx_end = skb->head + skb->end; //skb_end_pointer(skb);
#else
	pfhdr->rx_tail = skb->tail;
	pfhdr->rx_end = skb->end;
#endif

#ifdef CONFIG_SDIO_HCI
	//3 3. drop recvbuff
	if (pfhdr->precvbuf != NULL)
	{
		struct recv_buf *precvbuf = (struct recv_buf*)pfhdr->precvbuf;
		_irqL irql;
		...
		if (precvbuf->ref_cnt == 0) {
			_irqL irqL;
			_enter_critical(&adapter->recvpriv.free_recv_buf_queue.lock, &irqL);
			list_delete(&precvbuf->list);
			list_insert_tail(&precvbuf->list, get_list_head(&adapter->recvpriv.free_recv_buf_queue));
			adapter->recvpriv.free_recv_buf_queue_cnt++;
			_exit_critical(&adapter->recvpriv.free_recv_buf_queue.lock, &irqL);
		}
		_exit_critical_ex(&precvbuf->recvbuf_lock, &irql);
...
#ifdef PLATFORM_LINUX
#ifdef CONFIG_SDIO_HCI
	recvframe_expand_skb(adapter, prframe);
#endif
#endif
...
_func_enter_;		
	...
	if(ismfrag==1)
	{
		...
		{		
			...
			RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("Enqueuq: ismfrag = %d, fragnum= %d\n", ismfrag,fragnum));
		...
		else
		{
			...
			prtnframe=NULL;
			RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,("Free because pdefrag_q==NULL: ismfrag=%d, fragnum=%d\n", ismfrag, fragnum));
		}
	...
	if((ismfrag==0)&&(fragnum!=0))
	{
		...
		if(pdefrag_q != NULL)
		{
			...
			RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("defrag: ismfrag=%d, fragnum=%d\n", ismfrag, fragnum));
	...
	if((prtnframe!=NULL)&&(prtnframe->u.hdr.attrib.privacy))
	{
		...
		{
			RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,("recvframe_chkmic(padapter, prtnframe)==_FAIL\n"));
...
int recvbuf2recvframe(_adapter *padapter, struct recv_buf *precvbuf)
{
	...
	RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("+recvbuf2recvframe()\n"));
	...
	do{		
		...
		RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("rxdesc : offsset0:0x%08x, offsset4:0x%08x, offsset8:0x%08x, offssetc:0x%08x\n", 
		...
		RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("DRV_INFO_SIZE=%d\n", drvinfo_sz));

		if(pkt_len<=0)
		{
			//KeBugCheckEx(0x87120044, 0xe0, 0x4c, 0x87, 0x44);	
	
			RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("recvbuf2recvframe(), pkt_len<=0\n"));
		...
		if(precvframe==NULL)
		{
			...
			RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,("recvbuf2recvframe(), precvframe==NULL\n"));
...
_cont:
		...
		RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("recvbuf2recvframe(), transfer_len=%u, pkt_offset=%d, pkt_len=%d, ref_cnt=%d, precvbuf=0x%p\n",
		...
		{
			RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("recvbuf2recvframe(), recv_entry(precvframe) != _SUCCESS\n"));			
		}
...
int amsdu_to_msdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	while(a_len > ETH_HLEN) {
		...
		if( a_len < (ETHERNET_HEADER_SIZE + nSubframe_Length) ) {
			printk(KERN_ERR "nRemain_Length is %d and nSubframe_Length is : %d\n",a_len,nSubframe_Length);
			goto exit;
		}
		...
		if(nr_subframes >= MAX_SUBFRAME_COUNT) {
			printk(KERN_ERR "ParseSubframe(): Too many Subframes! Packets dropped!\n");
			break;
		}
...
int amsdu_to_msdu(_adapter *padapter, union recv_frame *prframe)
{
	...
	do{
	       ...
	       if (psnap->dsap==0xaa && psnap->ssap==0xaa && psnap->ctrl==0x03)
		{
			...
			{
  				 RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,("drop pkt due to invalid frame format!\n"));
		...
		if(eth_type==0x0800)//ip 
		{
			...
			{
				RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("@@@===recv tcp len:%d @@@===\n", pnrframe->u.hdr.len));
			}
		}	
#ifdef PLATFORM_OS_XP		
		else
...
//	u8	*prspbuf;
	...
	RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("+rxcmd_event_hdl\n"));
...
/*
 * Return
 *	_TRUE	enqueue packet
 *	_FALSE	indicate packet ok
 */
...
{	
	...
	if(!pattrib->amsdu)
	{
		...
		if(pattrib->qos !=1 /*|| pattrib->priority!=0 || IS_MCAST(pattrib->ra)*/)
		{
			if ((padapter->bDriverStopped ==_FALSE)&&( padapter->bSurpriseRemoved==_FALSE) )
			{
				RT_TRACE(_module_rtl8712_recv_c_, _drv_alert_, ("@@@@  recv_indicatepkt_reorder -recv_func recv_indicatepkt\n" ));	
...
void reordering_ctrl_timeout_handler(void *pcontext)
{
	...
	{
		u32 delay = REORDER_WAIT_TIME/2;
		if (padapter->recvpriv.free_recv_buf_queue_cnt == 0)
			delay = 1;
		_set_timer(&preorder_ctrl->reordering_ctrl_timer, delay);
	}
...
int process_recv_indicatepkts(_adapter *padapter, union recv_frame *prframe)
{
...
#endif	
	{
		...
		if(retval != _SUCCESS)
		{		
			RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,("wlanhdr_to_ethhdr: drop pkt \n"));
			return retval;
		}
		...
		{
			//indicate this recv_frame
			RT_TRACE(_module_rtl8712_recv_c_, _drv_notice_, ("@@@@ process_recv_indicatepkts- recv_func recv_indicatepkt\n" ));	
			recv_indicatepkt(padapter, prframe);	
		...
		{	
			RT_TRACE(_module_rtl8712_recv_c_, _drv_notice_, ("@@@@ process_recv_indicatepkts- recv_func free_indicatepkt\n" ));	

			RT_TRACE(_module_rtl8712_recv_c_, _drv_notice_, ("recv_func:bDriverStopped(%d) OR bSurpriseRemoved(%d)", padapter->bDriverStopped, padapter->bSurpriseRemoved));		
			retval = _FAIL;
			return retval;
		}	
...
static u8 evm_db2percentage(s8 value)
{
	...
	RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("EVMdbToPercentage92S Value=%d / %x \n", ret_val, ret_val));
...
//			cck_highpwr = FALSE;
	...
	else
	{
		...
		for(i=0; i<((padapter->registrypriv.rf_config) &0x0f ); i++)
		{
			...
			total_rssi += rssi;
			RT_TRACE(_module_rtl8712_recv_c_, _drv_err_, ("RF-%d RXPWR=%x RSSI=%d\n", i, rx_pwr[i], rssi));
		...
		pwdb_all = query_rx_pwr_percentage(rx_pwr_all);	
		RT_TRACE(_module_rtl8712_recv_c_, _drv_err_, ("PWDB_ALL=%d\n",	pwdb_all));
	...
	//	if(pMgntInfo->CustomerID != RT_CID_819x_Lenovo)		
		{
			...
			for(i=0; i<max_spatial_stream; i++)
			{
				...
				RT_TRACE(_module_rtl8712_recv_c_, _drv_err_, ("RXRATE=%x RXEVM=%x EVM=%s%d\n", 
 ...
 void process_link_qual(_adapter *padapter,union recv_frame *prframe)
{
	...
	if(pattrib->signal_qual != 0)	
	{
			...
			RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,("Total SQ=%d  pattrib->signal_qual= %d\n", padapter->recvpriv.signal_qual_data.total_val, pattrib->signal_qual));
		...
		RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,(" pattrib->signal_qual =%d\n", pattrib->signal_qual ));
...
//		Adapter->RxStats.RssiCalculateCnt++;	//For antenna Test
		...
		RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,("UI RSSI = %d, ui_rssi.TotalVal = %d, ui_rssi.TotalNum = %d \n", tmp_val, padapter->recvpriv.signal_strength_data.total_val,padapter->recvpriv.signal_strength_data.total_num));
	}
...
{	
	...
	pattrib = &prframe->u.hdr.attrib;	
       ...
       {     	       	
		...
		if(check_fwstate(pmlmepriv, WIFI_MP_LPBK_STATE) ==_FALSE){
			RT_TRACE(_module_rtl8712_recv_c_,_drv_alert_,("MP - Not in loopback mode , drop pkt \n"));
	...
	{ 		
		free_recvframe(orig_prframe, pfree_recv_queue);//free this recv_frame
		RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("recv_func: drop pkt \n"));
		goto _exit_recv_func;
	}
	process_phy_info(padapter, prframe);
	prframe=decryptor(padapter, prframe);
	if(prframe==NULL){
		RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("decryptor: drop pkt \n"));
		retval = _FAIL;
		goto _exit_recv_func;
	}
	...
	if(prframe==NULL){
		RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("recvframe_chk_defrag: drop pkt \n"));
		goto _exit_recv_func;
	}

	prframe=portctrl(padapter, prframe);
	if(prframe==NULL)	{
		RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("portctrl: drop pkt \n"));
		retval = _FAIL;
		goto _exit_recv_func;		
	}
...
//#if defined(CONFIG_80211N_HT) && defined(CONFIG_RECV_REORDERING_CTRL)
	...
	if(retval != _SUCCESS)
	{		
		RT_TRACE(_module_rtl8712_recv_c_,_drv_err_,("recv_func: process_recv_indicatepkts fail! \n"));
		free_recvframe(orig_prframe, pfree_recv_queue);//free this recv_frame
		goto _exit_recv_func;	
	}	
	...
	if(!pattrib->amsdu)
	{
		...
		if ((padapter->bDriverStopped ==_FALSE)&&( padapter->bSurpriseRemoved==_FALSE))
		{	
			RT_TRACE(_module_rtl8712_recv_c_, _drv_alert_, ("@@@@ recv_func: recv_func recv_indicatepkt\n" ));	
			//indicate this recv_frame
			recv_indicatepkt(padapter, prframe);
		}
		else
		{	
			RT_TRACE(_module_rtl8712_recv_c_, _drv_alert_, ("@@@@  recv_func: free_recvframe\n" ));		
			RT_TRACE(_module_rtl8712_recv_c_, _drv_debug_, ("recv_func:bDriverStopped(%d) OR bSurpriseRemoved(%d)", padapter->bDriverStopped, padapter->bSurpriseRemoved));		
			retval = _FAIL;
			free_recvframe(orig_prframe, pfree_recv_queue); //free this recv_frame
		}		
...
static int recvbuf2recvframe_u(_adapter *padapter, struct sk_buff *pskb)
{
	...
	RT_TRACE(_module_rtl8712_recv_c_,_drv_info_,("+recvbuf2recvframe_u()\n"));
...
{	
	...
	while(1)
	{
		...
		if(ret != _SUCCESS)		
		{
			...
			RT_TRACE(_module_rtl8712_recv_c_, _drv_debug_, ("recv_bh(): after recv_func(), ret=%d\n", ret));		
		}
	}
