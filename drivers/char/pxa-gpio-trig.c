	...
	if( param && 1 ){
		...
		if( pt ){
			...
			for( i = 0 ; i < pt->num_pins ; i++ ){
				...
				unsigned next = (0 != (GPLR(gpio) & GPIO_bit(gpio)));
				rval <<= 1 ;
				rval |= next ;
			}
	...
	switch (cmd){
		case GPIO_TRIG_CFG:{
			...
			if( 0 == copy_from_user(&trig, (void *)arg, sizeof(trig)) ){
				...
				set_irq_type(IRQ_GPIO(trig.trigger_pin), trig.rising_edge ? IRQ_TYPE_EDGE_RISING : IRQ_TYPE_EDGE_FALLING);	//pcmcia irq
