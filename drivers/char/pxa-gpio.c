...
int gpio_add_callback (void (*callback) (unsigned ,int), unsigned gpio)
{
	...
	if (callback) {
		struct dev_gpio* dev= GetAllocGpioDev(gpio,IN,1,IRQT_BOTHEDGE);
		if (dev) {
...
static void SetEdgeAndRegList(struct dev_gpio * dev)
{
	...
	edges = (flags | ((flags>>2) & IRQT_BOTHEDGE));
	edges &= (IRQT_BOTHEDGE|SAMPLE_FIQ);
	...
	if (i) {
		dev->gpio_edges = edges;
		if (i&IRQT_BOTHEDGE)
		set_irq_type(IRQ_GPIO(gpio), edges&IRQT_BOTHEDGE);
#ifdef CONFIG_ALLOW_FIQ
		if (i&SAMPLE_FIQ) {
			struct gpioData * g = g_gpioData;
	...
	edges &= IRQT_BOTHEDGE;
	if (edges == IRQT_BOTHEDGE) regList |= (1<<(gpio>>5));
	...
	if (g) {
		for (gpio=0; gpio<MAX_GPIO; gpio++) {
			...
			if (w) {
				...
				flags = (flags | (flags>>2)) & IRQT_BOTHEDGE;
			 ...
			 unsigned int cmd, unsigned long arg)
{
	...
	dev= GetAllocGpioDev(gpio,0,0,IRQT_BOTHEDGE);
	if (dev) {
		switch (cmd) {
#ifdef DEBOUNCE_LOGIC
